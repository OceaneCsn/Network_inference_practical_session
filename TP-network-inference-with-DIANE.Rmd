...
2. Deux critères de sélection sont disponibles: Le seuil de la valeur $p$ ajustée (False Discovery Rate) et le *Log Fold Change* peuvent tous deux être ajustés à la volée. Choisir les valeurs par défaut : 
...
Pour cela, DIANE utilise l'approche implémentée dans le logiciel Bioconductor [Coseq](https://www.bioconductor.org/packages/release/bioc/vignettes/coseq/inst/doc/coseq.html) [ref Rau et al 2018]. Coseq permet notamment 2 fonctionnalités utiles :
...
est GENIE3 [ref], une procédure d'apprentissage automatique qui a été parmi les meilleures méthodes des défis DREAM [ref].
...
Sélectionner une densité de 0.03. Dans ce TP, nous n'allons pas utiliser la fonctionalité de tests statistiques des arêtes inférées pour des raisons de temps de calcul, mais nous recommandons de l'utiliser pour des résultats d'inférence plus robustes et précis.
...
4. Vous pouvez choisir de regarder plus particulièrement certains gènes grâce au champs 'Gene ID to focus on" en hait à gauche du réseai, par exemples : 
...
7. Les distributions des degrés (indiquées dans le 3ème onglet) vous semblent-elles conformes aux attendus pour un réseau de régulation ? 
