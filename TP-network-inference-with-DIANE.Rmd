...
date: "TP inférence de réseau de gènes, 31 janvier - 7 février 2022"
output:
...
3. Une visualisation en heatmap d'une centaine de gènes des données est proposée dans la colonne centrale comme premier aperçu.
...
**(Optionnel, si vous devez un jour travailler avec vos propres données) Nouveau jeu de données d'expression**
...
1. La normalisation des données transcitpomiques est nécessaire pour corriger des biais de profondeur de séquençage entre les différents échantillons, et les rendre comparable dans la suite des analyses. Les données [@sewelam2020] utilisées dans ce TP sont présentées en Tags par millions (TPM) et ne nécessitent pas de normalisation. 
...
2. Les gènes trop faiblement sont généralement retirés de l'analyse car ils rendent l'estimation de paramètres statistiques trop incertaine, notamment pour les étpaes d'expression différentielle. Retirer de l'analyse les gènes dont le niveau d'expression est trop faible (section `Low counts filtering`, par défaut 10 x 24 conditions, cliquer sur `FILTER`). Observer sur les boxplots les différences de distribution des valeurs d'expression avant et après ce retrait de gènes.

## 3. Exploratory analysis

**3.1 PCA **

Les méthodes de réduction de la dimensionnalité sont fréquemment employées sur des données d'expression normalisées pour explorer comment les facteurs expérimentaux influencent l'expression des gènes, et pour
...
Choisir 1000 arbres. (En dehors du TP, si vous devez un jour analyser vos propres données et interprêter le réseau inféré, vous pouvez élever le nombre d'arbres à 4000 par exemple, pour consolider vos résultats. Notez que le temps de calcul sera d'autant plus élevé.)
...
*Note : Dans ce TP, nous n'allons pas utiliser la fonctionalité de tests statistiques des arêtes inférées pour des raisons de temps de calcul, mais nous recommandons de l'utiliser pour des résultats plus robustes et précis en vue de publier ou d'alimenter des interprétations biologiques.*
...
- un gène qui contient "osmotic" ou "heat" dans sa description (ils peuvent être fitrés dans le tableau des gènes avec l'option `Search` en haut à droite).
...
8. Visualisez les modules topologiques (groupes de gènes très densément connectés du réseau, détectées via l'algorithme de Louvain), en appyant sur le switch en haut à gauche intitulé `Nodes color`. Observer l'expression dans les modules (4ème onglet). 
